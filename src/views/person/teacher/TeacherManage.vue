<template>
  <div class="body">
    ...
    <a-card title="全部教师" style="margin: 24px">
      <template #extra>
    ...
    >
      ...
      >
        ...
        <a-form-item label="学号" name="teaid">
          <a-input v-model:value="formState.teaid" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
const onDelete = (key: any) => {
  Modal.confirm({
    ...
    onOk() {
      // 调用删除接口
      dataSource.value = dataSource.value.filter((item) => item.teaid !== key.teaid);
    },
    onCancel() {
      console.log("Cancel");
    },
